...
export function useTouchpadConfig({ configText, setConfigText }: TouchpadArgs) {
  ...
  // TOUCHPAD_SMOOTHING, which the backend no longer reads.
  ...
  // units, and -1 means "leave whatever the device already has" -- so that, not
  // 0, is the display fallback for an unset value.
  const touchOnThreshold = globalNum(keyName.TOUCHPAD_TOUCH_ON, -1)
  const touchOffThreshold = globalNum(keyName.TOUCHPAD_TOUCH_OFF, -1)
