...
export function useTouchpadConfig({ configText, setConfigText }: TouchpadArgs) {
  ...
  const mode = (pad?: Pad) => useMemo<TouchpadMode>(() => normalizeTouchpadMode(read(keyName.TOUCHPAD_MODE, pad)), [read, pad])
  const grid = (pad?: Pad) => useMemo(() => { const v = read(keyName.GRID_SIZE, pad)?.split(/\s+/).map(Number) ?? []; return { columns: Number.isFinite(v[0]) ? v[0] : 2, rows: Number.isFinite(v[1]) ? v[1] : 1 } }, [read, pad])
  const sensitivity = (pad?: Pad) => useMemo(() => { const n = Number.parseFloat(read(keyName.TOUCHPAD_SENS, pad) ?? ''); return Number.isFinite(n) ? n : undefined }, [read, pad])
  const value = (name: string, pad?: Pad) => useMemo(() => (read(name, pad) ?? '').trim().toUpperCase(), [read, name, pad])
